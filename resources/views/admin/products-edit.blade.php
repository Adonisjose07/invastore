        ...
        <div class="md:grid md:grid-cols-3 md:gap-6">
            <div class="md:col-span-1 mt-5">
                <div class="px-4 sm:px-0">
                    ...
                    <p class="mt-1 text-sm text-gray-600">Use this form to modify a product information. Then click save button located in bottom right.</p>
                </div>
            </div>
                        ...
                        <div class="bg-white px-4 py-5 sm:p-6">
                            <div class="grid grid-cols-1 gap-6">
                                ...
                                <div class="col-span-1">
                                    ...
                                    <div class="mt-1">
                                        <textarea id="product-description" name="product-description" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="Check this amazing collection of cake toppers">{{$product->description}}</textarea>
                                    </div>
            ...
            <div class="mt-5 md:col-span-2 md:mt-0">
                <form action="{{route('products.updateImages', $product->id)}}" method="POST" enctype="multipart/form-data">
                    @method('patch')
                    @csrf
                    <div class="overflow-hidden shadow sm:rounded-md">
                        <div class="bg-white px-4 py-5 sm:p-6">
                            <div class="grid grid-cols-1 gap-6">
                                <div class="col-span-1">
                                    ...
                                    <div class="mt-1 flex items-center">
                                        <span class="inline-block h-12 w-12 overflow-hidden rounded-full bg-gray-100">
                                            <?php $featured_image = false;?>
                                            @foreach ($product->gallery as $image)
                                                @if($image->featured == 1)
                                                    <?php $featured_image = $image;?>
                                                @endif
                                            @endforeach
                                            ...
                                            @if($featured_image != false)
                                                <img id="imgMainPhoto" class="" src="{{asset('storage/'.$featured_image->url)}}">
                                            @else
                                ...
                                <div class="col-span-1">
                                    ...
                                    <div class="uploadedGallery">
                                        @foreach ($product->gallery as $image)
                                            @if(!$image->featured)
                                                <div class="thumbsContainer px-2 border border-zinc-400 mb-4">
                                                    <img class="imgThumb" src="{{asset('storage/'.$image->url)}}">
                                                    <button type="button" class="uploadedGalleryButton bg-red-500 hover:bg-red-600 text-white rounded block mx-auto px-2 my-2" data-ref="{{$image->id}}">remove</button> 
                                                </div>
                                            @endif
                                        @endforeach
                                    </div>
    ...
    @push('scripts')
       <script>
            window.addEventListener('DOMContentLoaded', () => {
                document.querySelectorAll('.uploadedGalleryButton').forEach((el) => {
                    el.addEventListener('click', (e) => {
                        let url = "{{url('products/'.$product->id.'/images/delete')}}/"+e.target.dataset.ref;
                        ...
                        ajax.addEventListener('load', (evt) => {
                            ...
                            if(res.success){
                                document.querySelector("[data-ref='"+e.target.dataset.ref+"']").parentElement.remove();
                            }
                        })
                        ajax.send();
